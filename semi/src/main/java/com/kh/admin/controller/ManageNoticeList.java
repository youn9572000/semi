...
/**
 * Servlet implementation class ManageNoticeList
 */
@WebServlet("/admin/mlist")
public class ManageNoticeList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ManageNoticeList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new AmdminNoticeService().selectMListCount(1); // 1: 일반게시글 을 얻어오기위해 
		...
		List<Board> list = new AmdminNoticeService().selectBoardMList(pi);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("/views/adminPage/notice/ManageNotice.jsp").forward(request, response);
	}
