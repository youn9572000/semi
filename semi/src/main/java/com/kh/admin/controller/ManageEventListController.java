...
/**
 * Servlet implementation class ManageEventListController
 */
@WebServlet("/admin/elist")
public class ManageEventListController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ManageEventListController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		listCount = new AmdminNoticeService().selectEListCount(1); // 1: 일반게시글 을 얻어오기위해 
		...
		List<Board> list = new AmdminNoticeService().selectBoardEList(pi);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		
		request.getRequestDispatcher("/views/adminPage/notice/ManageEventBoard.jsp").forward(request, response);
	}
